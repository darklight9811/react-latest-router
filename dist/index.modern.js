import { createContext, useState, useCallback, useEffect, createElement, useContext, useMemo, Children, isValidElement, cloneElement } from 'react';
...
function Router(_ref) {
  ...
  var _React$useState = useState(window.location.pathname),
      current = _React$useState[0],
      setcurrent = _React$useState[1];

  var _React$useState2 = useState(_extends({}, bundle, guards)),
  ...
  var onProcessBack = useCallback(function () {
...
function Route(_ref) {
  ...
  var _React$useContext = useContext(RouterContext),
  ...
  var Component = useCallback(function () {
    ...
    if (response) {
      if (to !== null) {
        return createElement(to, _extends({}, props, response));
      }

      if (children !== null) {
        return createElement(children, _extends({}, props, response));
      }
    }
...
function Link(_ref) {
  ...
  var targetMemo = useMemo(function () {
    return "/" + to.replace(/(^\/|\/$)/, "");
  }, [to]);
  ...
  return createElement("a", Object.assign({}, domprop, {
    onClick: onClick,
    href: "#",
...
function Switch(_ref) {
  ...
  useEffect(function () {
    var compchildren = Children.toArray(props.children);
    ...
    for (var i = 0; i < compchildren.length; i++) {
      ...
      if (result) {
        ...
        if (typeof child.type === "function" && child.type.name == "Route") {
          if (isValidElement(renderable)) return setcomponent(cloneElement(renderable, _extends({}, renderable.props, restprops)));else return setcomponent(createElement(renderable, restprops));
        } else {
          if (isValidElement(child)) return setcomponent(cloneElement(child, _extends({}, child.props, restprops)));else return setcomponent(createElement(child, restprops));
        }
      }
    }
...
export { Link$1 as Link, Route$1 as Route, Router$1 as Router, RouterContext$1 as RouterContext, Switch$1 as Switch };
